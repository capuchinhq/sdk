...
// chat — an interactive session with an agent (`grove chat [name]`; no name talks to
...
func chat(args []string) error {
	...
	we, err := c.ExecuteWorkflow(ctx, client.StartWorkflowOptions{
		ID:        "agent:" + id,
		TaskQueue: grove.TaskQueue,
	}, "agentWorkflow", grove.StartInput{Agent: agentName, ConversationID: id})
	if err != nil {
		return err
	}
	wfID := we.GetID()
	...
	fmt.Printf("grove chat — %s (durable workflow: %s)\n", label, wfID)
	...
	for {
		...
		sawEvents := false
		for {
			st, err := withRetry(func() (grove.AgentState, error) { return queryState(c, wfID) })
			if err != nil {
				return err
			}

			events, err := withRetry(func() ([]grove.Event, error) { return queryEvents(c, wfID, offset) })
			if err != nil {
				return err
			}
			if len(events) > 0 {
			...
			if st.Status == "awaiting_approval" && len(st.PendingApprovals) > 0 {
				...
				for _, p := range st.PendingApprovals {
					...
					dec := grove.ApproveToolInput{ToolUseID: p.ToolUseID, Decision: "deny", Reason: "denied by user"}
					switch strings.ToLower(ans) {
					case "y", "yes":
						dec = grove.ApproveToolInput{ToolUseID: p.ToolUseID, Decision: "approve"}
					case "", "n", "no":
						// keep default deny
					default:
						dec.Reason = ans
...
func queryEvents(c client.Client, wfID string, since int) ([]grove.Event, error) {
	val, err := c.QueryWorkflow(context.Background(), wfID, "", "getEvents", since)
	if err != nil {
		return nil, err
	}
	var events []grove.Event
	err = val.Get(&events)
	return events, err
}

func queryState(c client.Client, wfID string) (grove.AgentState, error) {
	val, err := c.QueryWorkflow(context.Background(), wfID, "", "getState")
	if err != nil {
		return grove.AgentState{}, err
	}
	var st grove.AgentState
	err = val.Get(&st)
	return st, err
}

func printEvents(events []grove.Event) {
