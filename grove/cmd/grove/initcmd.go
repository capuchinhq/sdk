...
// initCmd — `grove init [dir]`: scaffold a runnable agent project. The template is
...
func initCmd(args []string) error {
	...
	if _, err := exec.LookPath("go"); err != nil {
		...
		fmt.Println("  go get github.com/rossnelson/grove-sdk/grove@latest")
	} else {
		fmt.Println("  resolving dependencies (go get)...")
		get := exec.Command("go", "get", "github.com/rossnelson/grove-sdk/grove@latest")
		get.Dir = dir
		if out, err := get.CombinedOutput(); err != nil {
			fmt.Printf("  go get failed:\n%s\n  Run it manually in %s when you're online:\n    go get github.com/rossnelson/grove-sdk/grove@latest\n", indent(string(out)), dir)
		} else {
	...
	fmt.Println("  grove dev          # the whole stack, your agent, hot reload")
	fmt.Println("  grove chat         # talk to it (another terminal)")
...
func main() {
	if err := grove.Serve(billing); err != nil {
		log.Fatal(err)
	}
}
`
...
A [Grove](https://grove.frnly.io) agent: tools + a prompt, running as a durable workflow.

## Run it

` + "```sh" + `
grove dev     # embedded Temporal + web UI + this worker, hot reload on save
grove chat    # talk to the agent (another terminal)
` + "```" + `
...
pauses for your approval. Kill grove dev mid-approval, restart, approve: the
