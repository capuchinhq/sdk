...
// TaskQueue is the Temporal task queue every grove worker serves.
const TaskQueue = "grove"

// DefaultAddress is where `grove dev` runs Temporal; Serve dials it unless
...
// the grove CLI uses this to host its embedded demo.
...
// `grove dev` default), serve the given agents, and block until interrupted.
func Serve(agents ...Agent) error {
	...
	var c client.Client
	var err error
	for i := 0; i < 20; i++ { // `grove dev` may still be booting Temporal
	...
	if err != nil {
		return fmt.Errorf("connect to temporal at %s (is `grove dev` running?): %w", addr, err)
	}
	defer c.Close()
	...
	fmt.Printf("grove worker — serving %s on %s (task queue %q)\n", strings.Join(names, ", "), addr, TaskQueue)
